...
public:
	...
	struct compare_node 
	{
		...
		bool operator()(const SLLNode* n1, const SLLNode* n2) const 
		{
	...
	typedef boost::heap::pairing_heap< SLLNode*, compare<SLLNode::compare_node> >::handle_type open_handle_t;
	open_handle_t open_handle;


	SLLNode();
	SLLNode(const SLLNode& other);
	SLLNode(int loc, int g_val, int h_val, SLLNode* parent, int timestep,
		 bool in_openlist = false);
	...
	struct eqnode 
	{
		bool operator()(const SLLNode* s1, const SLLNode* s2) const 
		{
	...
	struct NodeHasher 
	{
		std::size_t operator()(const SLLNode* n) const 
		{
