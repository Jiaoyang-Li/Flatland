...
void SinglePlanningFlat::updatePath(LLNode* goal)
{
	...
	for(int t = goal->g_val; t >= 0; t--)
	{
		...
		if (t >= goal->g_val){
            path[t].exit_heading = -1;
            path[t].exit_loc = curr->exit_loc;
		}
		else{
            path[t].exit_heading = path[t+1].heading;
            path[t].exit_loc = path[t+1].location;
		}
...
bool SinglePlanningFlat::search()
{
...
inline void SinglePlanningFlat::releaseClosedListNodes(hashtable_t* allNodes_table)
{
