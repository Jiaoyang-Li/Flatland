...
agent_priority_strategy = 3  #  choose a number between 0 and 5. Suggest 1
...
path = '/mnt/d/Flatland/test-round2/pkl/'
import os
import json
results = []
...
evaluation_number = 0  # evaluation counter
num_of_evaluations = 28  # total number of evaluations
total_time_limit = 2000  # unit: seconds
...
for folder in sorted(os.listdir(path)):
    ...
    for filename in os.listdir(path+folder):
        ...
        local_env = RailEnv(
            ...
            malfunction_generator_and_process_data= malfunction_from_file(test),  #malfunction_from_params(stochastic_data), #
        ...
        timelimit = 0  #remaining_time / (num_of_evaluations - evaluation_number + 1)
        ...
        print("initial runtime = ", time.time() - time0)
        while True:
            #####################################################################
            ...
            sum_of_costs += sum(all_rewards.values())
            if done['__all__']:
                print("Reward : ", sum_of_costs)
                print("sum of costs: ", - sum_of_costs + 2 * len(local_env.agents))
                finished = 0
                for i, a in enumerate(local_env.agents):
                    if(a.status == RailAgentStatus.DONE_REMOVED):
                        finished += 1
                print("steps = {}".format(steps))
                print("finished agents = {} ({}%)".format(finished, finished / len(local_env.agents)))
                CBS.clearMCP()
                print("total runtime = ", time.time() - time0)
