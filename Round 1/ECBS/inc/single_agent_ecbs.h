...
const CAT& res_table)
{
    ...
    int lastGoalConsTime = extractLastGoalTimestep(G.get_location(G.goal_ids[agent_id]), constraints);
    ...
    while ( !focal_list.empty() )
    {
        ...
        if (curr->id == G.goal_ids[agent_id] && curr->timestep > lastGoalConsTime)
        {
            updatePath(curr);
