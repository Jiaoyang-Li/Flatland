...
class FlatlandPost(Controller):
    ...
    def construct_stn(self):
        ...
        isin_mapf_node = np.zeros((self.n_agent, self.max_step), dtype=bool)

        # Create nodes and type1 edges
        for j in range(self.n_agent):
            self.mapf_node.append(self.encode_stn_node(self.path_list[j][0], j, 0))
            temp_node = self.mapf_node[0]
            for t in range(1, len(self.path_list[j])):
                if not self.path_list[j][t-1] == self.path_list[j][t]:
                    self.mapf_node.append(self.encode_stn_node(self.path_list[j][t], j, t))
                    self.mapf_edge.append((temp_node, self.mapf_node[t],
                                           {'lb': 1.0/self._speed_list[j], 'ub': np.inf}))
                    isin_mapf_node[j][t] = True
                    temp_node = self.mapf_node[t]
                        ...
                        if not j == k:
                            for t_k in range(t_j+1, len(self.path_list[k])):
                                if isin_mapf_node[k][t_k] and self.path_list[j][t_j] == self.path_list[k][t_k]:
                                    self.mapf_edge.append((self.encode_stn_node(self.path_list[j][t_j], j, t_j),
                                                           self.encode_stn_node(self.path_list[k][t_k], k, t_k),
                                                           {'lb': 1.0/self.speed_list[j], 'ub': np.inf}))
                                    break
        self.stn.node_list = self.mapf_node
        self.stn.edge_list = self.mapf_edge
...
if __name__ == '__main__':
    ...
    if args.config is None:
        ...
        temp_speed_list = np.random.choice([1.0, 1.0/2.0, 1.0/3.0], size=10)
        ...
        temp_speed_list = np.random.choice([1.0, 1.0/2.0, 1.0/3.0], size=config_dict['nr_agent'])
    my_post = FlatlandPost(in_env=env, in_prefix=file_prefix, speed_list=temp_speed_list)
